...
public class HashMapIndirecte<K, V> implements IHashMap<K, V> {
    ...
    @Override
    public boolean esBuida() { return mida == 0; }
    ...
    @Override
    public Iterator<V> iterator() { return new HashMapIterator<>(taula); }

    @Override
    public float factorCarrega() { return (float) longitud() / mida; }

    @Override
    public int midaTaula() { return mida; }

    private int obtenirIndex(K key) { return Math.abs(key.hashCode()) % mida; }
